<?php
namespace GLC\Client\Exception;
...
class Handler implements HandlerContract
{
    ...
    {
        if (in_array(config('auth.client.guard'), $e->guards())) {
    ...
    {
         if (View::exists($view = "client.errors.{$e->getStatusCode()}")) {
             if ($e->getStatusCode() == 503) {
                 return response()->view($view, ['message' => $e->getMessage()], 503);
             }
            return $this->makeViewResponse($view, $e);
        } else if (View::exists($view = 'client.errors.system')) {
            return $this->makeViewResponse($view, $e);
        }
        return null;
    }
