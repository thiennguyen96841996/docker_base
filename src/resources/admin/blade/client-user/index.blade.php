...
@section('CONTENTS')
    ...
    <h3>Client list</h3>
    <form method="GET" action="{{ route('admin.client-user.index') }}">
    ...
    <a href="{{ route('admin.client-user.create') }}">Create</a>
    <table>
        <thead>
        <tr>
            <th>Id</th>
        ...
        @forelse(Renderer::getPaginator() ?? [] as $val)
            <tr>
                <td><a href="{{ route('admin.client-user.show', $val->id) }}">{{ $val->id }}</a></td>
                <td>{{ $val->getName() }}</td>
                <td>{{ $val->getTel() }}</td>
                <td>{{ $val->email }}</td>
                <td>{{ \App\Common\Database\Definition\AvailableStatus::getName($val->is_available) }}</td>
                <td><a href="{{ route('admin.client-user.edit', $val->id) }}">Edit</a></td>
                <td>
                    <form method="POST" action="{{ route('admin.client-user.destroy', $val->id) }}">
                        @csrf
                        @method('DELETE')
