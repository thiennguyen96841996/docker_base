            ...
            <form method="POST" action="{{ route('admin.customer-user.updateConfirm',  $customerUser->id) }}" class="common-form">
                ...
                <div class="row g-2">
                    <div class="mb-3 col-md-6">
                        ...
                        <select name="status" class="form-select">
                            @foreach(\App\Common\Customer\Definition\CustomerStatus::cases() as $status)
                                @if ($status->value === Renderer::oldOrElse('status', $customerUser))
                                    <option value="{{$status->value}}" selected>{{\App\Common\Customer\Definition\CustomerStatus::getName($status->value)}}</option>
                                @else
                                    <option value="{{$status->value}}">{{\App\Common\Customer\Definition\CustomerStatus::getName($status->value)}}</option>
                                @endif
                            @endforeach
                        </select>
                    </div>
                ...
                <div class="row g-2">
                    ...
                    <div class="mb-3 col-md-6">
                        ...
                        <input type="text" name="gender" class="form-control" value="{{ \App\Common\Customer\Definition\CustomerGender::getName($customerUser->getGender()) }}" readonly>
                    </div>
                </div>
                <div class="row g-2">
                    <div class="mb-3 col-md-6">
