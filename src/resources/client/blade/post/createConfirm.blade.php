...
@section ('CONTENTS')
    ...
    <label>Avatar:</label>
    <img class="image" style="width: 192px; height: 130px;" src="{{ \App\Common\AWS\S3Service::display(\App\Common\Post\Definition\PostAvatar::POST_AVATAR_FOLDER_TMP . Auth::user()->id . '/' . request()->input('timestamp') . '/', request()->file('avatar')->getClientOriginalName()) }}" alt="" />

    <form method="POST" id="input_form" action="{{ route('client.post.store') }}">
        @csrf
        ...
        <div >
            <a id="btn_back_to_create" href="#" data-post-url="{{ route('client.post.create') }}" >back</a>
